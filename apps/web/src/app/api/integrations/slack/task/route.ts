...
/**
 * POST /api/integrations/slack/task
 * タスクに関するSlack通知を送信
 */
export async function POST(request: NextRequest) {
  try {
    const body = await request.json();
    const { channel, task, action } = body;
      ...
      return NextResponse.json(
        {
          error: {
            code: 'BAD_REQUEST',
            message: 'task and action are required',
          },
        },
        { status: 400 }
      );
    ...
    const validActions = ['created', 'updated', 'completed'];
    if (!validActions.includes(action)) {
      return NextResponse.json(
        {
          error: {
            code: 'BAD_REQUEST',
            message: 'action must be one of: created, updated, completed',
          },
        },
        { status: 400 }
      );
    ...
    await slackService.sendTaskNotification(slackChannel, task, action);
